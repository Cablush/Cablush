class Cadastros::EventosController < ApplicationController
  ...
  def index
    ...
    @title = I18n.t('views.cadastros.eventos_title', length: @eventos.length.to_s)
  end
  ...
  def edit
    ...
    @title = I18n.t('views.cadastros.editar_evento_title', evento: @evento.nome).html_safe
  end
  ...
  def evento_params
    params.require(:evento).permit(:nome, :descricao, :data, :hora, :data_fim, :website,
      :facebook, :flyer,
      esporte_ids: [],
      local_attributes: [:id, :latitude, :longitude, :logradouro, :numero, :complemento,
        :bairro, :cidade, :estado, :estado_nome, :cep, :pais])
  end
